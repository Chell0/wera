        ...
        <nav className="p-6 md:bg-transparent">
            <div className={`flex justify-between items-center ${isOpen ? 'bg-orange-400 p-4' : ''}`}>
                ...
                <div className="hidden md:block">
                    <Image src="/ossara-logo.png" alt="Ossara Logo" width={100} height={100} className="object-contain" />
                </div>
                <div className="md:hidden">
                    <button onClick={toggleMenu} className={`${isOpen ? 'text-white' : 'text-black'}`}>
                        {isOpen ? <XIcon className="h-8 w-8" /> : <MenuIcon className="h-8 w-8" />}
                    </button>
                </div>
            </div>
            {isOpen && (
                <div className="md:hidden mt-4 bg-orange-400 p-4 space-y-2">
