                ...
                <div className="flex flex-col justify-center md:flex-row md:flex-wrap md:items-center md:space-x-4">
                    {categories.map((category, index) => (
                        <button key={index} onClick={() => onCategorySelect(category.name)}
                            className="bg-orange-400 hover:bg-white hover:text-orange-400 text-white font-semibold py-2 px-4 mb-4 md:mb-0 rounded-lg shadow-sm transition duration-200">{category.name}
                        </button>
                    ))}
                    <button onClick={() => onCategorySelect(null)}
                        className="bg-white text-orange-400 hover:bg-orange-400 hover:text-white font-semibold py-2 px-4 mb-2 md:mb-0 rounded-lg shadow-sm transition duration-200 ml-0 md:ml-auto">View
                        All
                    </button>
                </div>
            </div>
